...
<?php if (count($this->alertas)) : ?>
    <script type="text/javascript">
        ...
        // Alert Notifyer
        var n = noty({
                text        : '<?php echo $alert['descripcion']; ?>',
                type        : 'warning',
                dismissQueue: true,
                layout      : 'topRight',
                theme       : 'defaultTheme',
                buttons     : [
                    {
                        addClass: 'btn btn-primary', text: '<?php echo JText::_('JR_PERSONAS_VERALERTAS'); ?>', onClick: function ($noty) {
                            $noty.close();                    
                            jQuery('#group117_tab').tab('show');
                        }
                    },
                    {
