...
class FabrikViewFormBase extends JView
{
	...
	protected function _loadTmplBottom(&$form)
	{
		...
		$form->resetButton = $params->get('reset_button', 0) && $this->editable == "1"
			? '<input type="reset" class="btn btn-default button" name="Reset" value="' . $params->get('reset_button_label') . '" />' : '';
		...
		$form->gobackButton = $params->get('goback_button', 0) == "1"
			? '<input type="button" class="btn btn-primary button" name="Goback" ' . $goBack . ' value="' . $params->get('goback_button_label')
				. '" />' : '';
		if ($model->isEditable() && $params->get('submit_button', 1))
		{
			...
			$form->submitButton = '<input type="' . $button . '" class="btn-success btn button ' . $submitClass . '" name="submit" value="'
